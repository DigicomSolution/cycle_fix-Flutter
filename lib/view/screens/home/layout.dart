...
class _LayoutState extends State<Layout> with SingleTickerProviderStateMixin {
  bool _exitDialogInterceptor(bool stopDefaultButtonEvent, RouteInfo info) {
    log('kSelectedScreen ${kTempScreens[selectedIndex]}');
    if ((appRoutes.location == AppRoutePath.home) && (selectedIndex == 0)) {
      // appRoutes.pop(context);
      // exit(0);
      onBackPressed(context);
    } else if ((appRoutes.location == AppRoutePath.home) &&
        (selectedIndex != 0)) {
      appRoutes.pop(context);
      // exit(0);
      // onBackPressed(context);
      ...
      if (appRoutes.location == AppRoutePath.singleInitiative) {
        appRoutes.pop(context);
      }
      log('appRoutes.configuration.routes.length ${appRoutes.configuration.routes}');
